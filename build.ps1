...
Write-Host "CursorCloak Build Script v1.0.1" -ForegroundColor Cyan
...
function Create-ZipPackages {
    ...
    Compress-Archive -Path ".\publish\framework\ui\*" -DestinationPath "CursorCloak-v1.0.1-win-x64.zip" -CompressionLevel Optimal -Force
    $frameworkSize = [math]::Round((Get-Item "CursorCloak-v1.0.1-win-x64.zip").Length / 1MB, 1)
    Write-Host "   Created CursorCloak-v1.0.1-win-x64.zip ($frameworkSize MB)" -ForegroundColor Green
    
    # Self-contained ZIP
    Compress-Archive -Path ".\publish\self-contained\ui\*" -DestinationPath "CursorCloak-v1.0.1-win-x64-selfcontained.zip" -CompressionLevel Optimal -Force
    $selfContainedSize = [math]::Round((Get-Item "CursorCloak-v1.0.1-win-x64-selfcontained.zip").Length / 1MB, 1)
    Write-Host "   Created CursorCloak-v1.0.1-win-x64-selfcontained.zip ($selfContainedSize MB)" -ForegroundColor Green
}
...
function Show-Summary {
    ...
    Write-Host "1. CursorCloak_Setup_v1.0.1.exe" -ForegroundColor Gray
    Write-Host "2. CursorCloak-v1.0.1-win-x64.zip" -ForegroundColor Gray
    Write-Host "3. CursorCloak_Setup_v1.0.1_SelfContained.exe" -ForegroundColor Gray
    Write-Host "4. CursorCloak-v1.0.1-win-x64-selfcontained.zip" -ForegroundColor Gray
    
    Write-Host ""
    Write-Host "File sizes:" -ForegroundColor Yellow
    if (Test-Path "CursorCloak-v1.0.1-win-x64.zip") {
        $size = [math]::Round((Get-Item "CursorCloak-v1.0.1-win-x64.zip").Length / 1MB, 1)
        Write-Host "Framework ZIP: $size MB" -ForegroundColor White
    }
    if (Test-Path "CursorCloak-v1.0.1-win-x64-selfcontained.zip") {
        $size = [math]::Round((Get-Item "CursorCloak-v1.0.1-win-x64-selfcontained.zip").Length / 1MB, 1)
        Write-Host "Self-contained ZIP: $size MB" -ForegroundColor White
    }
    if (Test-Path ".\Installer\CursorCloak_Setup_v1.0.1.exe") {
        $size = [math]::Round((Get-Item ".\Installer\CursorCloak_Setup_v1.0.1.exe").Length / 1MB, 1)
        Write-Host "Framework Installer: $size MB" -ForegroundColor White
    }
    if (Test-Path ".\Installer\CursorCloak_Setup_v1.0.1_SelfContained.exe") {
        $size = [math]::Round((Get-Item ".\Installer\CursorCloak_Setup_v1.0.1_SelfContained.exe").Length / 1MB, 1)
...
try {
    if ($Clean) {
        ...
        Write-Host "Clean completed. Use -CreateAllPackages to build." -ForegroundColor Cyan
        return
    }
    ...
    if ($CreateAllPackages) {
        ...
        Build-Solution
        Write-Host "Build completed. Use -CreateAllPackages to create deployment packages." -ForegroundColor Cyan
    }
