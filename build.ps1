...
function Show-Summary {
    ...
    if (Test-Path "CursorCloak-v1.0.0-win-x64.zip") {
        $size = [math]::Round((Get-Item "CursorCloak-v1.0.0-win-x64.zip").Length / 1MB, 1)
        Write-Host "Framework ZIP: $size MB" -ForegroundColor White
    }
    if (Test-Path "CursorCloak-v1.0.0-win-x64-selfcontained.zip") {
        $size = [math]::Round((Get-Item "CursorCloak-v1.0.0-win-x64-selfcontained.zip").Length / 1MB, 1)
        Write-Host "Self-contained ZIP: $size MB" -ForegroundColor White
    }
    if (Test-Path ".\Installer\CursorCloak_Setup_v1.0.0.exe") {
        $size = [math]::Round((Get-Item ".\Installer\CursorCloak_Setup_v1.0.0.exe").Length / 1MB, 1)
        Write-Host "Framework Installer: $size MB" -ForegroundColor White
    }
    if (Test-Path ".\Installer\CursorCloak_Setup_v1.0.0_SelfContained.exe") {
        $size = [math]::Round((Get-Item ".\Installer\CursorCloak_Setup_v1.0.0_SelfContained.exe").Length / 1MB, 1)
