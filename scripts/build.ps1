...
function Clear-BuildArtifacts {
    ...
    $cleanPaths = @(
        ...
        ".\publish",
        ".\Installer",
    ".\releases\CursorCloak-v1.1.0-*.zip"
    )
...
function Create-ZipPackages {
    ...
    Compress-Archive -Path ".\publish\framework\ui\*" -DestinationPath ".\releases\CursorCloak-v1.1.0-win-x64.zip" -CompressionLevel Optimal -Force
    $frameworkSize = [math]::Round((Get-Item ".\releases\CursorCloak-v1.1.0-win-x64.zip").Length / 1MB, 1)
    Write-Host "   Created releases\CursorCloak-v1.1.0-win-x64.zip ($frameworkSize MB)" -ForegroundColor Green
    
    # Self-contained ZIP
    Compress-Archive -Path ".\publish\self-contained\ui\*" -DestinationPath ".\releases\CursorCloak-v1.1.0-win-x64-selfcontained.zip" -CompressionLevel Optimal -Force
    $selfContainedSize = [math]::Round((Get-Item ".\releases\CursorCloak-v1.1.0-win-x64-selfcontained.zip").Length / 1MB, 1)
    Write-Host "   Created releases\CursorCloak-v1.1.0-win-x64-selfcontained.zip ($selfContainedSize MB)" -ForegroundColor Green
}
...
function Show-Summary {
    ...
    Write-Host "1. releases\CursorCloak_Setup_v1.1.0.exe" -ForegroundColor Gray
    Write-Host "2. releases\CursorCloak-v1.1.0-win-x64.zip" -ForegroundColor Gray
    Write-Host "3. releases\CursorCloak_Setup_v1.1.0_SelfContained.exe" -ForegroundColor Gray
    Write-Host "4. releases\CursorCloak-v1.1.0-win-x64-selfcontained.zip" -ForegroundColor Gray
    
    Write-Host ""
    Write-Host "File sizes:" -ForegroundColor Yellow
    if (Test-Path ".\releases\CursorCloak-v1.1.0-win-x64.zip") {
    $size = [math]::Round((Get-Item ".\releases\CursorCloak-v1.1.0-win-x64.zip").Length / 1MB, 1)
        Write-Host "Framework ZIP: $size MB" -ForegroundColor White
    }
    if (Test-Path ".\releases\CursorCloak-v1.1.0-win-x64-selfcontained.zip") {
    $size = [math]::Round((Get-Item ".\releases\CursorCloak-v1.1.0-win-x64-selfcontained.zip").Length / 1MB, 1)
        Write-Host "Self-contained ZIP: $size MB" -ForegroundColor White
    }
    if (Test-Path ".\releases\CursorCloak_Setup_v1.1.0.exe") {
    $size = [math]::Round((Get-Item ".\releases\CursorCloak_Setup_v1.1.0.exe").Length / 1MB, 1)
        Write-Host "Framework Installer: $size MB" -ForegroundColor White
    }
    if (Test-Path ".\releases\CursorCloak_Setup_v1.1.0_SelfContained.exe") {
    $size = [math]::Round((Get-Item ".\releases\CursorCloak_Setup_v1.1.0_SelfContained.exe").Length / 1MB, 1)
